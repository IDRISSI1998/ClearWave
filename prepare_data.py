...
def create_mixture_csv(args):
    ...
    all_speech_names = [na for na in os.listdir(speech_dir) if na.lower().endswith(".wav")]
...
def calculate_mixture_features(args):
    ...
    for i1 in range(1, len(lis)):
        ...
        elif len(speech_audio) < len(noise_audio):
            n_repeat = int(np.ceil(float(len(noise_audio)) / float(len(speech_audio))))
            speech_audio_ex = np.tile(speech_audio, n_repeat)
        ...
        # Write out normalized clean audio.
        out_bare_clear = os.path.join("%s-" %
            (os.path.splitext(speech_na)[0]))
        out_audio_path_clear = os.path.join(workspace, "normalized_clear_audios", "spectrogram",
            data_type, "%ddb" % int(snr), "%s.wav" % out_bare_clear)
        create_folder(os.path.dirname(out_audio_path_clear))
        write_audio(out_audio_path_clear, speech_audio, fs)
...
###
def calculate_noisy_features(args):
    """Calculate spectrogram for mixed, speech and noise audio. Then write the
    features to disk.
    ...
    # Extract spectrogram of all noisy_speech.
    noisy_speech_dir = os.path.join(noisy_dir)
    names = os.listdir(noisy_speech_dir)
    print(names)
    for (cnt, na) in enumerate(names):
        print(cnt, na)
        noisy_file_path = os.path.join(noisy_speech_dir, na)

        print(noisy_file_path)

        # Read noisy audio.
        (noisy_audio, _) = read_audio(noisy_file_path, target_fs=fs)

        # Extract spectrogram.
        mixed_complx_x = calc_sp(noisy_audio, mode='complex')
        speech_x = 1e-8
        noise_x = 1e-8
        alpha = 1e-8
        out_bare_na = na
...
if __name__ == '__main__':
    ...
    parser_create_mixture_csv.add_argument('--speechratio', type=int, default=1)
    ...
    parser_pack_features.add_argument('--calc_log', type=int, required=True)
    ...
    parser_calculate_noisy_features = subparsers.add_parser('calculate_noisy_features')
    parser_calculate_noisy_features.add_argument('--workspace', type=str, required=True)
    parser_calculate_noisy_features.add_argument('--noisy_dir', type=str, required=True)
    parser_calculate_noisy_features.add_argument('--data_type', type=str, required=True)
    parser_calculate_noisy_features.add_argument('--snr', type=float, required=True)
    ...
    elif args.mode == 'calculate_noisy_features':
        calculate_noisy_features(args)
